...
import { Avatar as HeroAvatar, Button, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Tooltip, Spinner } from "@heroui/react";
...
export default function Avatar({ src, name, size = "md", color = "default", isBordered = false, isEditable = false, onAvatarChange }: AvatarProps) {
  ...
  if (!isEditable) {
    return <HeroAvatar src={currentAvatar} name={name} size={size} color={color} isBordered={isBordered} />;
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <HeroAvatar src={currentAvatar} name={name} size={size} color={color} isBordered={isBordered} />
        
        {/* 加载状态 */}
        {isUploading && (
          <div className="absolute inset-0 bg-black bg-opacity-50 rounded-full flex items-center justify-center">
            <Spinner size="sm" color="default" classNames={{ circle1: "border-white", circle2: "border-white" }} />
          </div>
        )}
