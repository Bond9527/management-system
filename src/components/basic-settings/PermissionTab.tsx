          ...
          >
            <div className="flex items-center">
              ...
              <Checkbox
                ...
                isSelected={checked}
                onValueChange={() =>
                  !disabled && onCheck(node, !checked, parentKey || null)
                }
              >
                {node.label}
              </Checkbox>
...
export default function PermissionTab() {
  ...
  const menuItems = useMemo(
    () => convertApiMenuToPermissionTree(sidebarMenus || []),
    [sidebarMenus],
  );
  ...
  const getAllLabels = useCallback(
    (nodes: MenuItem[]): string[] =>
      nodes.flatMap((n) => [
        n.label,
        ...(n.children ? getAllLabels(n.children) : []),
      ]),
    [],
  );
  ...
  const allLabels = useMemo(
    () => getAllLabels(menuItems),
    [menuItems, getAllLabels],
  );
  ...
  const handleAddRole = () => {
    ...
    const existingRole = roles.find(
      (role) =>
        role.en.toLowerCase() === roleEn.toLowerCase() || role.cn === roleCn,
    );
  ...
  const handleCheck = (
    ...
    const getAll = (n: MenuItem): string[] => [
      n.label,
      ...(n.children ? n.children.flatMap(getAll) : []),
    ];
    ...
    if (checked) {
      ...
      if (parentKey) {
        ...
        if (
          parent &&
          parent.children &&
          parent.children.every((child) => newChecked.includes(child.label))
        ) {
  ...
  const handleCancel = () => {
    ...
    const selectedJobTitleName =
      jobTitles.find((jt) => jt.id.toString() === selectedJobTitle)?.name ||
      "未选择";
  ...
  const handleSave = async () => {
    ...
    showAlertMessage(
      "primary",
      "正在保存",
      `正在为"${selectedJobTitleName}"保存权限配置...`,
      0,
    );
    setSaving(true);
        ...
        <div className="space-y-6">
          ...
          {selectedJobTitle && (
            <div className="text-sm text-gray-600 pl-16">
              当前职称：
              {jobTitles.find((jt) => jt.id.toString() === selectedJobTitle)
                ?.name || "未找到"}
              (
              {jobTitles.find((jt) => jt.id.toString() === selectedJobTitle)
                ?.level || ""}
          ...
          <div>
            ...
            <div
              className={`border rounded p-4 ${!selectedJobTitle ? "bg-gray-100" : "bg-gray-50"}`}
            >
              {loading ? (
