...
import { Card, CardBody, CardHeader, Divider, Select, SelectItem, Button, Input } from '@heroui/react';
import { addToast } from "@heroui/toast";
import { useAuth } from '@/context/AuthContext';
import Avatar from '@/components/Avatar';
import { getJobTitles, updateUserJobTitle, JobTitle, getDepartments, updateUserDepartment, Department, updateUserEmployeeId, checkEmployeeIdExists } from '@/services/api';
...
export default function ProfilePage() {
  ...
  // 检查是否有变更
  useEffect(() => {
    const departmentChanged = selectedDepartment !== (user?.department_id?.toString() || '');
    const jobTitleChanged = selectedJobTitle !== (user?.job_title_id?.toString() || '');
  ...
  }, [selectedDepartment, selectedJobTitle, employeeId, updateUser, isUpdating, user]);
            ...
            <div className="flex flex-col items-center space-y-6">
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-small text-default-500">悬停头像进行编辑</p>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    >
                      {departments.map((dept) => (
                        <SelectItem key={dept.id.toString()} textValue={dept.name} aria-label={`部门: ${dept.name}`}>
                          {dept.name}
                        </SelectItem>
                      ))}
                    </Select>
                    {user.department && (
                      <p className="text-xs text-gray-500 mt-1">当前部门: {user.department}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 block">
                    ...
                    <Select
                      placeholder="请选择职称"
                      selectedKeys={selectedJobTitle ? new Set([selectedJobTitle]) : new Set()}
                      onSelectionChange={(keys) => {
                        const value = Array.from(keys)[0] as string;
                    ...
                    >
                      {jobTitles.map((title) => {
                        ...
                        return (
                          <SelectItem key={title.id.toString()} textValue={textValue} aria-label={`职称: ${textValue}`}>
                    ...
                    {user.job_title && (
                      <p className="text-xs text-gray-500 mt-1">当前职称: {user.job_title}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-2 block">
                    ...
                    {user.employee_id && !employeeIdError && (
                      <p className="text-xs text-gray-500 mt-1">当前工号: {user.employee_id}</p>
                    )}
                  </div>
                  <div>
