...
const SuppliesAddRecordPage: FC = () => {
  ...
  const [operationType, setOperationType] = useState<"in" | "out" | "adjust">(
    "in",
  );
  ...
  // 获取选中的耗材信息
  const selectedSupplyItem = supplies.find(
    (s: SupplyItem) => s.id.toString() === selectedSupply,
  );
      ...
      if (supply) {
        ...
        if (operationType === "out") {
          if (numQuantity > supply.current_stock) {
            ...
            supply.current_stock - numQuantity <=
            supply.safety_stock
          ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (operationType === "out" && numQuantity > supply.current_stock) {
        throw new Error(
          `❌ 出库数量不能超过当前库存！当前库存: ${supply.current_stock}${supply.unit}，申请数量: ${numQuantity}${supply.unit}`,
        );
      }
      ...
      setSuccessMessage(
        `${getOperationTypeText(operationType)}成功：${supply.name} ${numQuantity}${supply.unit}`,
      );
  ...
  const handleAddToBatch = () => {
    ...
    const existingIndex = batchRecords.findIndex(
      (record) => record.supplyId === selectedSupply,
    );
  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      for (const record of batchRecords) {
        try {
          const supply = supplies.find(
            (s: SupplyItem) => s.id.toString() === record.supplyId,
          );
          ...
          if (operationType === "out" && numQuantity > supply.current_stock) {
            throw new Error(
              `${supply.name}: 出库数量 ${numQuantity}${supply.unit} 超过当前库存 ${supply.current_stock}${supply.unit}`,
            );
          }
      ...
      if (successCount > 0) {
        ...
        setBatchRecords([]);
        setSuccessMessage(
          `批量${getOperationTypeText(operationType)}成功：共处理 ${successCount} 条记录`,
        );
        setShowSuccessModal(true);
      }
      ...
      addToast({
        title: "批量提交失败",
        description:
          error instanceof Error ? error.message : "批量提交失败，请重试",
        color: "danger",
        timeout: 5000,
      });
    } finally {
              ...
              <CardBody className="py-3 px-4">
                <div className="flex items-center gap-3">
                  <div className="flex-shrink-0">
                    ...
                    >
                      {stockInfo.change > 0 ? "+" : ""}
                      {stockInfo.change} {stockInfo.unit}
                    </Chip>
                  </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Select
                  ...
                  selectedKeys={
                    selectedSupply ? new Set([selectedSupply]) : new Set()
                  }
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        color={
                          selectedSupplyItem.current_stock <=
                          selectedSupplyItem.safety_stock
                            ? "danger"
                            : "success"
                        }
                        size="lg"
                      ...
                      >
                        {selectedSupplyItem.current_stock}{" "}
                        {selectedSupplyItem.unit}
                      </Badge>
                      {selectedSupplyItem.current_stock <=
                        selectedSupplyItem.safety_stock && (
                        <Tooltip
                          content={`安全库存: ${selectedSupplyItem.safety_stock}${selectedSupplyItem.unit}`}
                        >
                          <Chip
                            color="danger"
                            size="sm"
              ...
              <div className="pt-4 space-y-3">
                ...
                >
                  {isBatchMode
                    ...
                    : isSubmitting
                      ? "提交中..."
                      : `确认${getOperationTypeText(operationType)}`}
                </Button>
            ...
            <Card className="shadow-lg">
              <CardBody>
                <h3 className="text-lg font-semibold mb-4">
                  批量记录 ({batchRecords.length})
                </h3>
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {batchRecords.map((record) => {
                    const supply = supplies.find(
                      (s) => s.id.toString() === record.supplyId,
                    );
      ...
      >
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <Chip color="success" startContent={<PlusIcon />} variant="flat">
                操作成功
              </Chip>
            </div>
          </ModalHeader>
      ...
      >
        <ModalContent className="max-h-[90vh]">
          <ModalHeader className="flex flex-col gap-1 pb-2">
            <div className="flex items-center gap-2">
              <Chip color="primary" startContent={<PlusIcon />} variant="flat">
                新增耗材
              </Chip>
            </div>
          </ModalHeader>
