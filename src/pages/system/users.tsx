...
import { Search as SearchIcon, Plus as PlusIcon, Edit as EditIcon, Trash2 as DeleteIcon, Eye as EyeIcon, Key as KeyIcon, ChevronDown as ChevronDownIcon, MoreVertical as VerticalDotsIcon } from "lucide-react";
...
const INITIAL_VISIBLE_COLUMNS = ["name", "role", "department", "status", "actions"];
...
export default function UsersPage() {
  ...
  // 用户状态管理
  const [userStatusState, setUserStatusState] = useState<'active' | 'inactive' | 'disabled'>('active');
  ...
  // 工号重复检测（防抖）
  const checkEmployeeIdDuplicate = useCallback(async (employeeId: string, excludeUserId?: number) => {
    if (!employeeId.trim()) {
      setEmployeeIdError('');
      return;
    }
  ...
  // 权限检查函数
  const canManageUser = useCallback((targetUser: SystemUser): boolean => {
    if (!currentAuthUser) {
    ...
    if (currentAuthUser.username === 'admin' || 
  ...
  const handleCreateUser = async () => {
    ...
    try {
      const userData = {
        ...createFormData,
        department: createFormData.department ? parseInt(createFormData.department) : null,
        job_title: createFormData.job_title ? parseInt(createFormData.job_title) : null,
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      const userData = {
        ...editFormData,
        department: editFormData.department ? parseInt(editFormData.department) : null,
        job_title: editFormData.job_title ? parseInt(editFormData.job_title) : null,
      ...
      // 如果状态有变化，同时更新状态
      const currentStatus = currentUser.profile?.status || (currentUser.is_active ? 'active' : 'disabled');
      if (userStatusState !== currentStatus) {
        await api.post(`/users/${currentUser.id}/toggle_status/`, { status: userStatusState });
      }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        ...
        const errorMessage = error.response?.data?.error || error.message || "重置密码失败";
        addToast({
          title: "错误",
          description: errorMessage,
          color: "danger",
  ...
  // 渲染单元格
  const renderCell = useCallback((user: SystemUser, columnKey: React.Key): React.ReactNode => {
    ...
    switch (columnKey) {
      case "name":
        ...
        const getAvatarColor = (user: SystemUser) => {
          ...
          if (user.profile?.job_title) {
            ...
            // 初级职位 - 默认色
            if (jobTitle.includes('技术员') || jobTitle.includes('助理') || 
                jobTitle.includes('事务员') || jobTitle.includes('初级')) {
              return "default";
            }
          }
        ...
        return (
          <User
            avatarProps={{
              ...
              src: user.profile?.avatar_url || `https://api.dicebear.com/7.x/initials/svg?seed=${user.username}`
            }}
      ...
      case "role":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-small capitalize">{user.profile?.role || "未分配"}</p>
            <p className="text-bold text-tiny capitalize text-default-400">{user.profile?.department || "无部门"}</p>
          </div>
        );
      case "department":
        return (
      ...
      case "status":
        const getStatusColor = (status: string) => {
          switch (status) {
            case 'active': return 'success';    // 激活 - 绿色
            case 'inactive': return 'default';  // 未激活 - 灰色
            case 'disabled': return 'danger';   // 禁用 - 红色
            ...
            <Dropdown>
              ...
              <DropdownMenu>
                ...
                <DropdownItem 
                  key="delete" 
                  isDisabled={!hasPermission}
                  className={!hasPermission ? "text-default-300" : "text-danger"} 
          ...
          <div className="flex gap-3">
            <Dropdown>
              <DropdownTrigger className="hidden sm:flex">
                <Button endContent={<ChevronDownIcon className="text-small w-4 h-4" />} variant="flat">
                  状态
                </Button>
              </DropdownTrigger>
              <DropdownMenu
            ...
            <Button color="primary" endContent={<PlusIcon className="w-4 h-4" />} onPress={onCreateOpen}>
              添加用户
            </Button>
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">共 {totalUsers} 个用户</span>
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button 
                  variant="flat" 
                  size="sm"
                  endContent={<ChevronDownIcon className="text-small w-3 h-3" />}
                  className="min-w-16"
                >
                  {rowsPerPage}
                </Button>
  ...
  return (
    <div className="p-6">
      <Card>
        ...
        <CardBody>
          ...
          >
            ...
            <TableBody emptyContent={"没有找到用户"} items={sortedItems} isLoading={loading}>
              {(item) => (
      ...
      {/* 创建用户模态框 */}
      <Modal isOpen={isCreateOpen} onOpenChange={onCreateClose} size="2xl" scrollBehavior="inside">
        <ModalContent>
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select
                  label="部门"
                  placeholder="请选择部门"
                  selectedKeys={createFormData.department ? [createFormData.department] : []}
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setCreateFormData({...createFormData, department: selectedKey || ""});
                  }}
                >
                ...
                <Select
                  label="职称"
                  placeholder="请选择职称"
                  selectedKeys={createFormData.job_title ? [createFormData.job_title] : []}
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setCreateFormData({...createFormData, job_title: selectedKey || ""});
                  }}
                >
                  {jobTitles.map((jt) => (
                    <SelectItem key={jt.id.toString()} textValue={`${jt.name} (${jt.level})`}>
                      {jt.name} ({jt.level})
                    </SelectItem>
                  ))}
                </Select>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select
                  ...
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setCreateFormData({...createFormData, role: selectedKey || ""});
                  }}
                >
                  {roles.map((role) => (
              ...
              <div className="flex gap-4">
                <Switch
                  ...
                  onValueChange={(checked) => setCreateFormData({...createFormData, is_active: checked})}
                >
                  激活状态
                </Switch>
                <Switch
                  isSelected={createFormData.is_staff}
                  onValueChange={(checked) => setCreateFormData({...createFormData, is_staff: checked})}
                >
                  员工状态
                </Switch>
                <Switch
                  isSelected={createFormData.is_superuser}
                  onValueChange={(checked) => setCreateFormData({...createFormData, is_superuser: checked})}
                >
                  超级用户
                </Switch>
              </div>
          ...
          <ModalFooter>
            ...
            <Button color="primary" onPress={handleCreateUser} isDisabled={!!employeeIdError}>
              创建
            </Button>
          </ModalFooter>
        </ModalContent>
      ...
      {/* 编辑用户模态框 */}
      <Modal isOpen={isEditOpen} onOpenChange={onEditClose} size="2xl" scrollBehavior="inside">
        <ModalContent>
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select
                  label="部门"
                  placeholder="请选择部门"
                  selectedKeys={editFormData.department ? [editFormData.department] : []}
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setEditFormData({...editFormData, department: selectedKey || ""});
                  }}
                >
                  {departments.map((dept) => (
                    <SelectItem key={dept.id.toString()} textValue={dept.name}>
                ...
                <Select
                  label="职称"
                  placeholder="请选择职称"
                  selectedKeys={editFormData.job_title ? [editFormData.job_title] : []}
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setEditFormData({...editFormData, job_title: selectedKey || ""});
                  }}
                >
                  {jobTitles.map((jt) => (
                    <SelectItem key={jt.id.toString()} textValue={`${jt.name} (${jt.level})`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select
                  ...
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as string;
                    setEditFormData({...editFormData, role: selectedKey || ""});
                  }}
                >
                  {roles.map((role) => (
                    <SelectItem key={role.id.toString()} textValue={role.name}>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-small font-medium mb-2 block">用户状态</label>
                  <div className="flex gap-4">
          ...
          <ModalFooter>
            ...
            <Button color="primary" onPress={handleUpdateUser} isDisabled={!!employeeIdError}>
              更新
            </Button>
          </ModalFooter>
        </ModalContent>
      ...
      {/* 查看用户模态框 */}
      <Modal isOpen={isViewOpen} onOpenChange={onViewClose} size="lg">
        <ModalContent>
              ...
              <div className="grid gap-4">
                <div className="flex items-center gap-4">
                  <User
                    avatarProps={{
                      radius: "lg",
                      src: currentUser.profile?.avatar_url || `https://api.dicebear.com/7.x/initials/svg?seed=${currentUser.username}`,
                      size: "lg"
                    }}
                    description={currentUser.email}
                    name={currentUser.first_name && currentUser.last_name ? `${currentUser.first_name} ${currentUser.last_name}` : currentUser.username}
                  />
                  <div>
                    <Chip 
                      color={
                        currentUser.profile?.status === 'active' ? "success" :
                        currentUser.profile?.status === 'inactive' ? "default" : "danger"
                      } 
                      variant="flat"
                    >
                      {currentUser.profile?.status_display || (currentUser.is_active ? "激活" : "禁用")}
                    </Chip>
                  </div>
                </div>
                <Divider />
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{currentUser.profile?.role || "未分配"}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{currentUser.profile?.job_title || "未分配"}</p>
                  </div>
                  <div>
                    <p className="text-small text-default-500">工号</p>
                    <p className="font-medium">{currentUser.profile?.employee_id || "未填写"}</p>
                  </div>
                  <div>
                    <p className="text-small text-default-500">电话</p>
                    <p className="font-medium">{currentUser.profile?.phone || "未填写"}</p>
                  </div>
                  <div>
                    <p className="text-small text-default-500">员工状态</p>
                    <p className="font-medium">{currentUser.is_staff ? "是" : "否"}</p>
                  </div>
                  <div>
                    <p className="text-small text-default-500">超级用户</p>
                    <p className="font-medium">{currentUser.is_superuser ? "是" : "否"}</p>
                  </div>
                  <div>
