...
export function validateDataConsistency(
  ...
  supplies.forEach((supply) => {
    const supplyRecords = records.filter(
      (record) => record.supply === supply.id,
    );
    ...
    if (supply.current_stock > 0) {
      if (calculatedStock < 0) {
        issues.push(
          `耗材 "${supply.name}" 计算库存为负数: ${calculatedStock}${supply.unit}`,
        );
      }
    }
...
export function generateInventorySummary(
  ...
  const summary: InventorySummary = {
    ...
    lowStockItems: safeSupplies.filter((s) => s.current_stock <= s.safety_stock)
      .length,
    recentActivity: safeRecords.filter((r) => new Date(r.timestamp) >= weekAgo)
      .length,
    totalStock: safeSupplies.reduce((sum, item) => sum + item.current_stock, 0),
    categories: Array.from(new Set(safeSupplies.map((item) => item.category)))
      .length,
    byCategory: {},
  };
